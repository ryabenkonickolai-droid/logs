...
This role is based on the original Alloy role and has been extended with additional custom tasks to satisfy project-specific requirements.

The following custom tasks have been added:

rsync-json – synchronizes JSON configuration files required for Alloy integration and operation.
logrotate – configures log rotation policies for Alloy logs to prevent uncontrolled disk space consumption.

These tasks are integrated into the role execution workflow and are executed as part of the deployment process.

## Customisation

This Ansible role installs and configures Alloy, which can be used to collect traces, metrics, and logs.
This role is tailored for operating systems such as Ubuntu and Debian.
...
This role is tailored for operating systems such as **Ubuntu**, **Debian**.
